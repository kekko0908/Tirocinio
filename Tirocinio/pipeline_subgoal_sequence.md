...
## FSM State Diagrams (detailed, readable layout)
...
stateDiagram-v2
    ...
    state EXPLORE {
        [*] --> BuildContext
        BuildContext: aggiorna sensor + memoria
        note right of BuildContext
          - sensor: distanze, pos, yaw
          - object_memory + behavior_memory
          - context_summary (map, checkpoints, agent)
        end note

        BuildContext --> ProbeGate
        ProbeGate: VLM probe every N steps?
        ProbeGate --> ProbeVLM: yes
        ProbeGate --> SkipProbe: no

        ProbeVLM: VLM "probe_scene"
        note right of ProbeVLM
          - se hint forte: orienta subito
          - se hint debole: enqueue scan
        end note
        ProbeVLM --> DecideHint

        SkipProbe --> DecideHint

        DecideHint: è arrivato un hint direzionale?
        DecideHint --> ApplyHint: yes
        DecideHint --> ExplorePolicy: no

        ApplyHint: rotazione/movimento guidato
        ApplyHint --> StepAction

        ExplorePolicy: scansione + movimenti di esplorazione
        note right of ExplorePolicy
          - scan_degrees / scan_cooldown
          - action_mgr.enqueue_probe_scan
        end note
        ExplorePolicy --> StepAction

        StepAction: esegui action (Move/Rotate)
    }

    EXPLORE --> ToSEARCH: target visto o probabile
    EXPLORE --> ToNAV: nav_need_plan attivo
    EXPLORE --> ToAPPROACH: target abbastanza vicino
    EXPLORE --> ToFAIL: max_steps o blocco
...
stateDiagram-v2
    ...
    state SEARCH {
        ...
        BuildContext: aggiorna sensor + memoria
        note right of BuildContext
          - mem_summary + object_memory
          - context_summary (ego + map)
        end note
        ...
        ProbeVLM: VLM "probe_scene"
        ProbeVLM --> DecideAction
        SkipProbe --> DecideAction

        DecideAction: scegli azione o scan
        DecideAction --> StepAction
        StepAction: esegui action (Move/Rotate)
    }

    SEARCH --> ToAPPROACH: search_confirmed
    SEARCH --> ToNAV: det_dist > navigate_dist
    SEARCH --> ToEXPLORE: search_timeout
...
stateDiagram-v2
    ...
    state NAVIGATE {
        [*] --> EnsurePlan
        EnsurePlan: nav_need_plan? calcola path + checkpoints
        note right of EnsurePlan
          - pathfinding su reachable
          - linea bianca + checkpoint blu
          - context_summary include map + checkpoints
        end note

        EnsurePlan --> MaybeReplan
        MaybeReplan: no_progress >= threshold?
        MaybeReplan --> ForceReplan: yes
        MaybeReplan --> ChooseAction: no

        ForceReplan: reset nav_subgoals/queue
        ForceReplan --> LookDownGate
        LookDownGate: dist_front < safe_front?
        LookDownGate --> LookDown: yes
        LookDownGate --> ChooseAction: no

        LookDown: enqueue LookDown + replan
        LookDown --> ChooseAction

        ChooseAction: VLM choose_action con doppia immagine
        note right of ChooseAction
          - ego view + topdown
          - segue linea bianca / checkpoint
        end note
        ChooseAction --> StepAction

        StepAction: esegui Move/Rotate
    }

    NAVIGATE --> ToAPPROACH: dist <= approach_dist
    NAVIGATE --> ToSEARCH: target perso / scan
    NAVIGATE --> ToFAIL: max_steps o blocco
...
stateDiagram-v2
    ...
    state SEARCH_NEAR {
        [*] --> CheckAnchor
        CheckAnchor: aggiorna anchor (near reference)
        note right of CheckAnchor
          - anchor position da object_memory
          - anchor_dist calcolata da sensori
        end note

        CheckAnchor --> AnchorGate
        AnchorGate: anchor valida e vicina?
        AnchorGate --> AnchorMissing: no
        AnchorGate --> ScanNear: yes

        AnchorMissing: fallback a SEARCH globale
        AnchorMissing --> ExitSearch

        ScanNear: scansione + probe vicino anchor
        ScanNear --> DecideConfirm

        DecideConfirm: search_confirmed?
        DecideConfirm --> ToApproach: yes
        DecideConfirm --> ToSearch: no/timeout
    }

    SEARCH_NEAR --> SEARCH: near_anchor_missing
    SEARCH_NEAR --> SEARCH: near_anchor_far
    SEARCH_NEAR --> APPROACH: search_confirmed
    SEARCH_NEAR --> SEARCH: near_timeout
```
...
stateDiagram-v2
    ...
    state APPROACH {
        [*] --> AcquireTarget
        AcquireTarget: usa oracle/YOLO per target
        note right of AcquireTarget
          - centroid/bbox da masks o det
          - distanza da metadata o depth
        end note

        AcquireTarget --> RecoveryGate
        RecoveryGate: collision / last_action_success?
        RecoveryGate --> Recovery: yes
        RecoveryGate --> Centering: no

        Recovery: move laterale o rotazione
        Recovery --> Centering

        Centering: allinea target al centro
        Centering --> DistanceCheck

        DistanceCheck: close_enough?
        DistanceCheck --> ConfirmClose: yes
        DistanceCheck --> StepAhead: no

        ConfirmClose: approccio confermato
        ConfirmClose --> StepAction
        StepAhead: MoveAhead con step adattivo
        StepAhead --> StepAction

        StepAction: esegui action
    }

    APPROACH --> ToLOCALIZE: approach_close
    APPROACH --> ToNAV: approach_stuck
    APPROACH --> ToSEARCH: lost_target
...
stateDiagram-v2
    ...
    state LOCALIZE {
        [*] --> ConfirmTarget
        ConfirmTarget: esegui YOLO/visione per conferma
        note right of ConfirmTarget
          - aggiorna localize_hits
          - salva trace di azioni
        end note

        ConfirmTarget --> ConfirmGate
        ConfirmGate: localize_confirmed?
        ConfirmGate --> Done: yes
        ConfirmGate --> TimeoutGate: no

        TimeoutGate: superato max_localize_steps?
        TimeoutGate --> BackToSearch: yes
        TimeoutGate --> ConfirmTarget: no
    }

    LOCALIZE --> SELECT_TARGET: target_done
    LOCALIZE --> SEARCH: localize_timeout
```
