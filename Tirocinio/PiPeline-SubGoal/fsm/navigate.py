...
from pipeline_modules.constants import LOOKDOWN_DEGREES
...
def parse_nav_plan(raw: Dict[str, Any], max_steps: int) -> List[Dict[str, Any]]:
    ...
    if isinstance(plan, dict) and plan.get("action"):
        plan = [plan]
    if plan is None and raw.get("action"):
        plan = [raw]
    ...
    for item in plan:
        ...
        if action in {"RotateLeft", "RotateRight", "LookDown"}:
            ...
            if action == "LookDown" and "degrees" not in spec:
                spec["degrees"] = int(LOOKDOWN_DEGREES)
