...
class VLMEngine:
    def __init__(self, model_id: str, action_set: List[str]):
        ...
        if torch.cuda.is_available():
            ...
            dtype = torch.float32
            device_map = None
        model_id_lc = str(model_id).lower()
        use_qwen2_vl = "qwen2" in model_id_lc and "vl" in model_id_lc
        if use_qwen2_vl:
            if Qwen2VLForConditionalGeneration is not None:
                ...
                raise RuntimeError(
                    "Qwen2-VL requires transformers with vision-language support. "
                    "Upgrade transformers (and accelerate) to a newer version."
                )
        elif AutoModelForImageTextToText is not None:
            try:
                self.model = AutoModelForImageTextToText.from_pretrained(
    ...
    def choose_action(
        ...
        topdown_image: Optional[Image.Image] = None,
    ):
        """
